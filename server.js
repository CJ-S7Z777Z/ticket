...
app.post('/submit', upload.array('files[]', 10), async (req, res) => {
    try {
        ...
        if (data.Материал) {
            message += `*Материал:* ${data.material}\n`;
        }
        message += `*Количество:* ${data.quantity}\n`;

        if (data['installLuvvers'] === 'on') {
            message += `*Установка люверсов:* Да\n`;
            message += `*Количество люверсов:* ${data['luvversCount'] || 0}\n`;
        } else {
